// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using SplitwiseDemo.Data;

namespace SplitwiseDemo.Migrations
{
    [DbContext(typeof(SystemDBContext))]
    [Migration("20180813121919_Create7")]
    partial class Create7
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "2.1.1-rtm-30846")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("SplitwiseDemo.Models.Bill", b =>
                {
                    b.Property<int>("billid")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<DateTime>("bill_created_at");

                    b.Property<int?>("bill_created_byuserid");

                    b.Property<DateTime>("bill_date");

                    b.Property<DateTime>("bill_updated_at");

                    b.Property<int?>("bill_updated_byuserid");

                    b.Property<string>("description");

                    b.Property<int?>("groupid");

                    b.Property<double>("total_amount");

                    b.HasKey("billid");

                    b.HasIndex("bill_created_byuserid");

                    b.HasIndex("bill_updated_byuserid");

                    b.HasIndex("groupid");

                    b.ToTable("bills");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.Currency", b =>
                {
                    b.Property<int>("currencyid")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("currency_name");

                    b.Property<string>("currency_symbol");

                    b.HasKey("currencyid");

                    b.ToTable("currencies");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.Friend", b =>
                {
                    b.Property<int>("friendid")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("friend_touserid");

                    b.Property<bool>("splitwise_status");

                    b.Property<int?>("user_iduserid");

                    b.HasKey("friendid");

                    b.HasIndex("friend_touserid");

                    b.HasIndex("user_iduserid");

                    b.ToTable("Friend");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.Group", b =>
                {
                    b.Property<int>("groupid")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("group_created_by");

                    b.Property<DateTime>("group_created_on");

                    b.Property<string>("group_name");

                    b.Property<bool>("is_simplified_depts");

                    b.HasKey("groupid");

                    b.HasIndex("group_created_by");

                    b.ToTable("groups");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.GroupMember", b =>
                {
                    b.Property<int>("memberid")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("groupid");

                    b.Property<int?>("userid");

                    b.HasKey("memberid");

                    b.HasIndex("groupid");

                    b.HasIndex("userid");

                    b.ToTable("groupmembers");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.Language", b =>
                {
                    b.Property<int>("languageid")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("language_name");

                    b.HasKey("languageid");

                    b.ToTable("languages");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.Payer", b =>
                {
                    b.Property<int>("payerid")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<double>("amount_paid");

                    b.Property<int>("bill_id");

                    b.Property<int>("paid_by");

                    b.HasKey("payerid");

                    b.HasIndex("bill_id");

                    b.HasIndex("paid_by");

                    b.ToTable("payers");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.Settelments", b =>
                {
                    b.Property<int>("settelmentid")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("bill_id");

                    b.Property<double>("paid_amount");

                    b.Property<DateTime>("paid_on");

                    b.Property<int?>("payeeuserid");

                    b.Property<int?>("payeruserid");

                    b.HasKey("settelmentid");

                    b.HasIndex("bill_id");

                    b.HasIndex("payeeuserid");

                    b.HasIndex("payeruserid");

                    b.ToTable("settelments");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.SharedWith", b =>
                {
                    b.Property<int>("sharedid")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("bill_id");

                    b.Property<double>("owes_amount");

                    b.Property<int?>("owes_touserid");

                    b.Property<int?>("shared_withuserid");

                    b.HasKey("sharedid");

                    b.HasIndex("bill_id");

                    b.HasIndex("owes_touserid");

                    b.HasIndex("shared_withuserid");

                    b.ToTable("sharedwiths");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.Timezone", b =>
                {
                    b.Property<int>("timezoneid")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("timezone_name");

                    b.HasKey("timezoneid");

                    b.ToTable("timezones");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.User", b =>
                {
                    b.Property<int>("userid")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<DateTime>("created_on");

                    b.Property<int?>("currencyid");

                    b.Property<string>("email_id");

                    b.Property<int?>("languageid");

                    b.Property<string>("password");

                    b.Property<string>("phone_no");

                    b.Property<byte[]>("profile_pic");

                    b.Property<int?>("timezoneid");

                    b.Property<DateTime>("updated_on");

                    b.Property<string>("user_name");

                    b.HasKey("userid");

                    b.HasIndex("currencyid");

                    b.HasIndex("languageid");

                    b.HasIndex("timezoneid");

                    b.ToTable("users");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.Bill", b =>
                {
                    b.HasOne("SplitwiseDemo.Models.User", "bill_created_by")
                        .WithMany("bill_creater")
                        .HasForeignKey("bill_created_byuserid");

                    b.HasOne("SplitwiseDemo.Models.User", "bill_updated_by")
                        .WithMany("bill_updater")
                        .HasForeignKey("bill_updated_byuserid");

                    b.HasOne("SplitwiseDemo.Models.Group", "group_id")
                        .WithMany("bills")
                        .HasForeignKey("groupid");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.Friend", b =>
                {
                    b.HasOne("SplitwiseDemo.Models.User", "friend_to")
                        .WithMany("friend_to")
                        .HasForeignKey("friend_touserid");

                    b.HasOne("SplitwiseDemo.Models.User", "user_id")
                        .WithMany("user_friend")
                        .HasForeignKey("user_iduserid");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.Group", b =>
                {
                    b.HasOne("SplitwiseDemo.Models.User", "user")
                        .WithMany("group")
                        .HasForeignKey("group_created_by")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("SplitwiseDemo.Models.GroupMember", b =>
                {
                    b.HasOne("SplitwiseDemo.Models.Group", "group_id")
                        .WithMany("groupmembers")
                        .HasForeignKey("groupid");

                    b.HasOne("SplitwiseDemo.Models.User", "user_id")
                        .WithMany("groupmembers")
                        .HasForeignKey("userid");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.Payer", b =>
                {
                    b.HasOne("SplitwiseDemo.Models.Bill", "bill")
                        .WithMany("payers")
                        .HasForeignKey("bill_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("SplitwiseDemo.Models.User", "user")
                        .WithMany("payers")
                        .HasForeignKey("paid_by")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("SplitwiseDemo.Models.Settelments", b =>
                {
                    b.HasOne("SplitwiseDemo.Models.Bill", "bill")
                        .WithMany("settelments")
                        .HasForeignKey("bill_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("SplitwiseDemo.Models.User", "payee")
                        .WithMany("settelmentpayee")
                        .HasForeignKey("payeeuserid");

                    b.HasOne("SplitwiseDemo.Models.User", "payer")
                        .WithMany("settelmentpayer")
                        .HasForeignKey("payeruserid");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.SharedWith", b =>
                {
                    b.HasOne("SplitwiseDemo.Models.Bill", "bill")
                        .WithMany("sharedwiths")
                        .HasForeignKey("bill_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("SplitwiseDemo.Models.User", "owes_to")
                        .WithMany("billpayee")
                        .HasForeignKey("owes_touserid");

                    b.HasOne("SplitwiseDemo.Models.User", "shared_with")
                        .WithMany("billpayer")
                        .HasForeignKey("shared_withuserid");
                });

            modelBuilder.Entity("SplitwiseDemo.Models.User", b =>
                {
                    b.HasOne("SplitwiseDemo.Models.Currency", "currency_id")
                        .WithMany("user")
                        .HasForeignKey("currencyid");

                    b.HasOne("SplitwiseDemo.Models.Language", "language_id")
                        .WithMany("user")
                        .HasForeignKey("languageid");

                    b.HasOne("SplitwiseDemo.Models.Timezone", "timezone_id")
                        .WithMany("user")
                        .HasForeignKey("timezoneid");
                });
#pragma warning restore 612, 618
        }
    }
}
